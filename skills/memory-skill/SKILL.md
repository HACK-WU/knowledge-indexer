...
对目标 Group 执行 `ki query-group --scope <对应scope> --groups "目标Group路径" --mode hot,emerging`
...
所有写入统一使用 `ki sync-relation`，scope 区分项目记忆（`${scope}-memory`）和用户画像（`user-profile`）。
...
**流程**：查找现有内容 → 确认 → 覆盖写入（`sync-relation` 同名覆盖）

1. `ki query-group --groups "目标Group路径" --mode hot,emerging` → 找到目标 Relation
2. `ki get-module-info` → 取现有内容确认
3. `ki sync-relation` 同名覆盖写入新内容
4. 刷新缓存
