...
# 方式 2：使用 npm scripts
npm run manage-index -- --scope <scope> --action create-root --root-name <name>
```
...
# 方式 2：使用 npm scripts
npm run manage-index -- --scope <scope> --action create --parent <父节点路径> --name <子节点名称>
```
...
# 方式 2：使用 npm scripts
npm run manage-index -- --scope my-project --action create --parent "设计文档" --name "API"
```
...
# 方式 2：使用 npm scripts
npm run manage-index -- --scope <scope> --action delete --parent <父节点路径> --name <子节点名称> [--force]
```
...
# 方式 2：使用 npm scripts
npm run sync-relation -- --scope my-project --group "设计文档/API" --relation "用户登录" --module-info "# 用户登录\n\n## 流程\n1. 输入账号密码\n2. 验证\n3. 返回token" --keywords "登录,认证,token"
```
...
# 方式 2：使用 npm scripts
npm run sync-relation -- --scope <scope> --input <jsonFile>
```
