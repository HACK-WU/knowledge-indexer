...
flowchart TD
    ...
    I --> J{命中记忆?}
    J -- 是 --> K[提取摘要去掉路径段<br/>→ ki_sync_relation 回写本地]
    K --> G
    J -- 否 --> L[回问用户<br/>补充线索]
    L --> H
```
...
> `module_info` 使用 `memory_recall` 返回的**摘要文本**，不额外调用 `ki_get_module_info` 取原文。
...
**注意**：`ki_sync_relation` 只写 relations-cache + local KB，**不写 memory**。AI 是否写 memory 自行决定。
...
| 🔴 4 | `keywords` 中出现未在 `module_info` 原文中出现的词 |
| 🔴 5 | 跨 scope 串数据 |
